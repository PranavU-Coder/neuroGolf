...
def show_legend():
    ...
    ax.text(
        5.75,
        3.1,
        "<--- special colors to indicate one-hot encoding errors --->",
        color="black",
    )
...
def single_layer_conv2d_network(weight_fn, kernel_size):
    ...
    weight_cells = itertools.product(
        range(_CHANNELS), range(_CHANNELS), kernel_offsets, kernel_offsets
    )
    ...
    node_def = onnx.helper.make_node(
        "Conv", ["input", "W"], ["output"], kernel_shape=kernel_shape, pads=pads
    )
...
def verify_network(network, task_num, examples):
    ...
    try:
        ...
        return
    arc_agi_right, arc_agi_wrong, arc_agi_expected = verify_subset(
        session, examples["train"] + examples["test"]
    )
    arc_gen_right, arc_gen_wrong, arc_gen_expected = verify_subset(
        session, examples["arc-gen"]
    )
    ...
    if macs is None or memory is None or params is None:
        ...
        print()
        print(
            f"It appears to require {macs} MACs + {memory} bytes + {params} params, yielding {points:.3f} points."
        )
        print()
        print("Next steps:")
        print(
            f" * Click the link below to download {filename} onto your local machine."
        )
